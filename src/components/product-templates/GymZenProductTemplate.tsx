...
export default function GymZenProductTemplate({ product, business, relatedProducts, waLink, waNumber }: Props) {
  // Theme specific styles
  const primaryColor = business.primaryColor || (['bg-zinc-900','bg-black','bg-zinc-950'].includes('bg-stone-100') ? '#ffffff' : '#000000');
  const secondaryColor = business.secondaryColor || 'stone-100';

  return (
    <div className="min-h-screen bg-stone-100 text-stone-800 font-sans transition-colors duration-500">
      {/* Header */}
              ...
              {waNumber ? (
                product.inStock ? (
                  <a
                    ...
                    className="block w-full text-center text-lg font-bold py-4 transition-all flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-600/90 text-white rounded-xl shadow-lg shadow-emerald-600/20 hover:-translate-y-1"
