...
export default function PlayfulProductTemplate({ product, business, relatedProducts, waLink, waNumber }: Props) {
  // Theme specific styles
  const primaryColor = business.primaryColor || (['bg-zinc-900','bg-black','bg-zinc-950'].includes('bg-pink-50') ? '#ffffff' : '#000000');
  const secondaryColor = business.secondaryColor || 'pink-50';

  return (
    <div className="min-h-screen bg-pink-50 text-purple-900 font-sans transition-colors duration-500">
      {/* Header */}
              ...
              {waNumber ? (
                product.inStock ? (
                  <a
                    ...
                    className="block w-full text-center text-lg font-bold py-4 transition-all flex items-center justify-center gap-2 bg-pink-500 hover:bg-pink-500/90 text-white rounded-xl shadow-lg shadow-pink-500/20 hover:-translate-y-1"
