...
export default function ServiceModernProductTemplate({ product, business, relatedProducts, waLink, waNumber }: Props) {
  // Theme specific styles
  const primaryColor = business.primaryColor || (['bg-zinc-900','bg-black','bg-zinc-950'].includes('bg-slate-900') ? '#ffffff' : '#000000');
  const secondaryColor = business.secondaryColor || 'slate-900';

  return (
    <div className="min-h-screen bg-slate-900 text-slate-50 font-sans transition-colors duration-500">
      {/* Header */}
      ...
      <main className="max-w-6xl mx-auto px-6 py-12 md:py-24">
        <div className="grid md:grid-cols-2 gap-12 lg:gap-24 items-center">
          ...
          {/* Product Image */}
          <div className="aspect-square relative overflow-hidden rounded-2xl shadow-xl bg-white/5 group">
            {product.imageUrl ? (
              <img
              ...
              {waNumber ? (
                product.inStock ? (
                  <a
                    ...
                    className="block w-full text-center text-lg font-bold py-4 transition-all flex items-center justify-center gap-2 bg-blue-500 hover:bg-blue-500/90 text-white rounded-xl shadow-lg shadow-blue-500/20 hover:-translate-y-1"
