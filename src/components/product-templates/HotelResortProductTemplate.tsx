...
export default function HotelResortProductTemplate({ product, business, relatedProducts, waLink, waNumber }: Props) {
  // Theme specific styles
  const primaryColor = business.primaryColor || (['bg-zinc-900','bg-black','bg-zinc-950'].includes('bg-cyan-50') ? '#ffffff' : '#000000');
  const secondaryColor = business.secondaryColor || 'cyan-50';

  return (
    <div className="min-h-screen bg-cyan-50 text-cyan-950 font-sans transition-colors duration-500">
      {/* Header */}
              ...
              {waNumber ? (
                product.inStock ? (
                  <a
                    ...
                    className="block w-full text-center text-lg font-bold py-4 transition-all flex items-center justify-center gap-2 bg-cyan-600 hover:bg-cyan-600/90 text-white rounded-xl shadow-lg shadow-cyan-600/20 hover:-translate-y-1"
