...
export default function HotelLuxuryProductTemplate({ product, business, relatedProducts, waLink, waNumber }: Props) {
  // Theme specific styles
  const primaryColor = business.primaryColor || (['bg-zinc-900','bg-black','bg-zinc-950'].includes('bg-zinc-900') ? '#ffffff' : '#000000');
  const secondaryColor = business.secondaryColor || 'zinc-900';

  return (
    <div className="min-h-screen bg-zinc-900 text-amber-50 font-serif transition-colors duration-500">
      {/* Header */}
      ...
      <main className="max-w-6xl mx-auto px-6 py-12 md:py-24">
        <div className="grid md:grid-cols-2 gap-12 lg:gap-24 items-center">
          ...
          {/* Product Image */}
          <div className="aspect-square relative overflow-hidden rounded-xl shadow-lg bg-white/5 group">
            {product.imageUrl ? (
              <img
              ...
              {waNumber ? (
                product.inStock ? (
                  <a
                    ...
                    className="block w-full text-center text-lg font-bold py-4 transition-all flex items-center justify-center gap-2 bg-transparent text-amber-500 border border-amber-500 hover:bg-amber-500 hover:text-black rounded-sm uppercase tracking-widest"
                    style={{}}
                  >
                  ...
                  <div className="block w-full text-center text-lg font-bold py-4 rounded-xl cursor-not-allowed bg-zinc-300 text-zinc-500">
