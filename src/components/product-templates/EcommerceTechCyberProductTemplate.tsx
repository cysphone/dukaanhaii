...
export default function EcommerceTechCyberProductTemplate({ product, business, relatedProducts, waLink, waNumber }: Props) {
  // Theme specific styles
  const primaryColor = business.primaryColor || (['bg-zinc-900','bg-black','bg-zinc-950'].includes('bg-[#0f172a]') ? '#ffffff' : '#000000');
  const secondaryColor = business.secondaryColor || '[#0f172a]';

  return (
    <div className="min-h-screen bg-[#0f172a] text-cyan-50 font-mono transition-colors duration-500">
      {/* Header */}
      ...
      <main className="max-w-6xl mx-auto px-6 py-12 md:py-24">
        <div className="grid md:grid-cols-2 gap-12 lg:gap-24 items-center">
          ...
          {/* Product Image */}
          <div className="aspect-square relative overflow-hidden rounded-xl border border-cyan-500 shadow-[0_0_30px_rgba(var(--cyan-500-rgb),0.3)] bg-white/5 group">
            {product.imageUrl ? (
              <img
              ...
              {waNumber ? (
                product.inStock ? (
                  <a
                    ...
                    className="block w-full text-center text-lg font-bold py-4 transition-all flex items-center justify-center gap-2 bg-black text-cyan-500 border border-cyan-500 hover:shadow-[0_0_20px_rgba(var(--cyan-500-rgb),0.5)] hover:bg-cyan-500/10 rounded-xl"
                    style={{}}
                  >
