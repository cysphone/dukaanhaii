...
export default function FuturisticProductTemplate({ product, business, relatedProducts, waLink, waNumber }: Props) {
  // Theme specific styles
  const primaryColor = business.primaryColor || (['bg-zinc-900','bg-black','bg-zinc-950'].includes('bg-black') ? '#ffffff' : '#000000');
  const secondaryColor = business.secondaryColor || 'black';

  return (
    <div className="min-h-screen bg-black text-cyan-50 font-sans transition-colors duration-500">
      {/* Header */}
      <header className="border-b border-white/10 sticky top-0 backdrop-blur-xl z-50 bg-inherit/80">
      ...
      <main className="max-w-6xl mx-auto px-6 py-12 md:py-24">
        <div className="grid md:grid-cols-2 gap-12 lg:gap-24 items-center">
          ...
          {/* Product Image */}
          <div className="aspect-square relative overflow-hidden rounded-xl border border-cyan-400 shadow-[0_0_30px_rgba(var(--cyan-400-rgb),0.3)] bg-white/5 group">
            {product.imageUrl ? (
              <img
              ...
              {waNumber ? (
                product.inStock ? (
                  <a
                    ...
                    className="block w-full text-center text-lg font-bold py-4 transition-all flex items-center justify-center gap-2 bg-black text-cyan-400 border border-cyan-400 hover:shadow-[0_0_20px_rgba(var(--cyan-400-rgb),0.5)] hover:bg-cyan-400/10 rounded-xl"
                    style={{}}
                  >
                  ...
                  <div className="block w-full text-center text-lg font-bold py-4 border border-zinc-600 bg-zinc-900 text-zinc-600 cursor-not-allowed rounded-xl">
      ...
      {relatedProducts.length > 0 && (
        <section className="bg-white/5 py-16 md:py-32">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
              {relatedProducts.map(rp => (
                <Link key={rp.id} href={getProductUrl(business.slug, rp.id)} className="group block relative">
                  <div className="aspect-[4/5] bg-white/5 border-white/10 border relative overflow-hidden rounded-xl mb-4">
                    {rp.imageUrl ? (
                      <img
                        src={rp.imageUrl}
                        alt={rp.name}
      ...
      {/* Footer */}
      <footer className="border-t border-white/10 py-12 md:py-24 text-center">
