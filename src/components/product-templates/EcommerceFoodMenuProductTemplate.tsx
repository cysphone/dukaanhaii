...
export default function EcommerceFoodMenuProductTemplate({ product, business, relatedProducts, waLink, waNumber }: Props) {
  // Theme specific styles
  const primaryColor = business.primaryColor || (['bg-zinc-900','bg-black','bg-zinc-950'].includes('bg-orange-50') ? '#ffffff' : '#000000');
  const secondaryColor = business.secondaryColor || 'orange-50';

  return (
    <div className="min-h-screen bg-orange-50 text-orange-950 font-serif transition-colors duration-500">
      {/* Header */}
      ...
      <main className="max-w-6xl mx-auto px-6 py-12 md:py-24">
        <div className="grid md:grid-cols-2 gap-12 lg:gap-24 items-center">
          ...
          {/* Product Image */}
          <div className="aspect-square relative overflow-hidden rounded-xl shadow-lg bg-black/5 group">
            {product.imageUrl ? (
              <img
              ...
              {waNumber ? (
                product.inStock ? (
                  <a
                    ...
                    className="block w-full text-center text-lg font-bold py-4 transition-all flex items-center justify-center gap-2 bg-orange-600 hover:bg-orange-600/90 text-white rounded-xl shadow-lg shadow-orange-600/20 hover:-translate-y-1"
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
              {relatedProducts.map(rp => (
                <Link key={rp.id} href={getProductUrl(business.slug, rp.id)} className="group block relative">
                  <div className="aspect-[4/5] bg-black/5 border-black/10 border relative overflow-hidden rounded-xl mb-4">
                    {rp.imageUrl ? (
                      <img
                        src={rp.imageUrl}
                        alt={rp.name}
