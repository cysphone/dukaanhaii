...
export default function HotelBoutiqueProductTemplate({ product, business, relatedProducts, waLink, waNumber }: Props) {
  // Theme specific styles
  const primaryColor = business.primaryColor || (['bg-zinc-900','bg-black','bg-zinc-950'].includes('bg-rose-50') ? '#ffffff' : '#000000');
  const secondaryColor = business.secondaryColor || 'rose-50';

  return (
    <div className="min-h-screen bg-rose-50 text-rose-950 font-serif transition-colors duration-500">
      {/* Header */}
              ...
              {waNumber ? (
                product.inStock ? (
                  <a
                    ...
                    className="block w-full text-center text-lg font-bold py-4 transition-all flex items-center justify-center gap-2 bg-rose-600 hover:bg-rose-600/90 text-white rounded-xl shadow-lg shadow-rose-600/20 hover:-translate-y-1"
