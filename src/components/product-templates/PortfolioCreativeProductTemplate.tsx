...
export default function PortfolioCreativeProductTemplate({ product, business, relatedProducts, waLink, waNumber }: Props) {
  // Theme specific styles
  const primaryColor = business.primaryColor || (['bg-zinc-900','bg-black','bg-zinc-950'].includes('bg-purple-900') ? '#ffffff' : '#000000');
  const secondaryColor = business.secondaryColor || 'purple-900';

  return (
    <div className="min-h-screen bg-purple-900 text-white font-sans transition-colors duration-500">
      {/* Header */}
      ...
      <main className="max-w-6xl mx-auto px-6 py-12 md:py-24">
        <div className="grid md:grid-cols-2 gap-12 lg:gap-24 items-center">
          ...
          {/* Product Image */}
          <div className="aspect-square relative overflow-hidden rounded-[3rem] shadow-2xl bg-white/5 group">
            {product.imageUrl ? (
              <img
              ...
              {waNumber ? (
                product.inStock ? (
                  <a
                    ...
                    className="block w-full text-center text-lg font-bold py-4 transition-all flex items-center justify-center gap-2 bg-fuchsia-400 hover:bg-fuchsia-400/90 text-white rounded-xl shadow-lg shadow-fuchsia-400/20 hover:-translate-y-1"
