...
export default function MinimalProductTemplate({ product, business, relatedProducts, waLink, waNumber }: Props) {
  // Theme specific styles
  const primaryColor = business.primaryColor || (['bg-zinc-900','bg-black','bg-zinc-950'].includes('bg-white') ? '#ffffff' : '#000000');
  const secondaryColor = business.secondaryColor || 'white';
              ...
              {waNumber ? (
                product.inStock ? (
                  <a
                    ...
                    className="block w-full text-center text-lg font-bold py-4 transition-all flex items-center justify-center gap-2 bg-brand-600 hover:bg-brand-600/90 text-white rounded-xl shadow-lg shadow-brand-600/20 hover:-translate-y-1"
