  ...
  switch (session.step) {
    case 'ask_add_product':
      if (text.toUpperCase() === 'YES' || text.toUpperCase() === 'HAAN' || text === '1') {
        replyText = `Great! 📝 Enter your new service's *Name*:`;
        nextStep = 'collect_product_name';
      } else {
        replyText = `Your site is ready! 🎉\n\nType *MENU* to go back to the main menu.`;
        nextStep = 'completed';
      }
      break;
    ...
    case 'collect_product_name':
      ...
      replyText = `Nice! 🏷️\n\nNow enter the service *Price* or *Hourly Rate* (numbers only, e.g., 500):`;
    ...
    case 'ask_product_desc_ai':
      if (text === '1') {
        ...
        const generated = await generateProductDescription(
          { name: collectedData.productName, price: collectedData.productPrice, businessName: bizName, category: collectedData.category || 'Service' },
          generationIndex
        );
        ...
        const creditNote = creditsLeft > 0 ? `_(You have ${creditsLeft} regeneration${creditsLeft > 1 ? 's' : ''} left for this service)_` : `_(No regenerations left)_`;
        replyText = `✨ *Here's your AI-generated service description:*\n\n"${generated}"\n\n${creditNote}\n\nReply:\n✅ *YES* - Use this description\n🔄 *REGENERATE* - Generate a different one`;
        nextStep = 'ai_product_desc_pending';
      } else if (text === '2') {
        replyText = `📝 Please describe the service (1-2 lines):`;
        nextStep = 'collect_product_desc';
      } else if (text === '3') {
        collectedData.productDesc = '';
        replyText = `Done! 📸\n\nFinal step: Please send a *Cover Photo* for this service.`;
    ...
    case 'ai_product_desc_pending': {
      if (msgUpper === 'YES') {
        ...
        if (collectedData.aiCredits.productDesc <= 0) {
          ...
          const creditsLeft = collectedData.aiCredits.productDesc;
          const creditNote = creditsLeft > 0 ? `_(You have ${creditsLeft} regeneration${creditsLeft > 1 ? 's' : ''} left for this service)_` : `_(No regenerations left. Reply YES to accept or REGENERATE to see your options.)_`;
    ...
    case 'collect_product_image':
      if (message.image && message.image.id) {
        ...
        replyText = `Please send an actual *Photo (Image)* of the service.`;
        nextStep = 'collect_product_image';
      }
      break;
    ...
    case 'ai_image_pending':
      if (text === '1' || msgUpper === 'YES') {
        replyText = `Awesome! Adding service to your site... ⏳`;
    ...
    case 'collect_additional_images': {
      if (msgUpper === 'DONE' || msgUpper === 'NO') {
        replyText = `Got it! Would you like to add another service? Reply *YES* or *NO*.`;
